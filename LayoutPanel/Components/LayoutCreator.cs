namespace zUI
{
	...
	public class LayoutCreator : MonoBehaviour
	{
		...
		public void CreateSetup()
		{
			...
			var columnParentRect = LayoutEditorUtilities.CreateHoritontalOrVertical(
			...
			for (int i = 0; i < columnParentRect.childCount; i++)
			{
				...
				border.borderHideMode = hideCreatedBordersInHierarchy ? BorderHideMode.Hidden : BorderHideMode.Visible;
		...
		void Reset()
		{
			...
			if (canvas == null)
			{
				...
				EditorApplication.delayCall += () => Destroy(this);
				return;
			}
		...
		private void AddRandomChildren(GameObject parent, int count, ref List<LayoutPanel> panels)
		{
			for (int j = 0; j < count; j++)
			{
				var panel = CreatePanel(
					parent,
					"Item [" + (panels.Count + 1) + " ] " + PrimitiveExtensions.RandomString(3));
				panel.freeMode = false;
				panels.Add(panel);
			}
		}

		private void SetColors(List<LayoutPanel> panels)
		{
		...
		LayoutPanel CreatePanel(GameObject parent, string newName = "NoName")
		{
			...
			creator.borderHideMode = hideCreatedBordersInHierarchy ? BorderHideMode.Hidden : BorderHideMode.Visible;
		...
		{
			LayoutEditorUtilities.CreateHoritontalOrVertical(
				GetComponent<RectTransform>(),
				LayoutDirection.Horizontal,
				columns,
				0.01f);
