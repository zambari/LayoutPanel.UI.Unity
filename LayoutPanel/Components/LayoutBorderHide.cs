	...
	public class LayoutBorderHide : MonoBehaviour
	{
		...
		public BorderHideMode borderHideMode
		{
			...
			set
			{
				...
				else hiddenCount = SetBasedOnRoot(transform);
				...
				else
				{
					...
					else name = name.SetTag("   ┌" + hiddenCount + "┐");
				}
		...
		{
			Debug.Log(
				"warning, setting visibility based on getcomponentinchildren: this only works on active objects",
				gameObject);
			...
			{
				foreach (var t in tops) t.gameObject.hideFlags = HideFlags.None;
				foreach (var b in bs) b.gameObject.hideFlags = HideFlags.None;
			}
		...
		int SetBasedOnRoot(Transform source)
		{
			...
			for (int i = 0; i < source.childCount; i++)
			{
				...
				{
					if (_borderHideMode == BorderHideMode.Visible) thisGame.hideFlags = HideFlags.None;
					else
					{
						hiddenCount++;
		...
		private void Reset()
		{
			for (int i = 0; i < 41; i++) UnityEditorInternal.ComponentUtility.MoveComponentUp(this);
			...
			else borderColor = Color.gray * 0.6f + (new Color(Random.value, Random.value, Random.value)) * 0.3f;
		}
#endif
		...
		private void OnValidate()
		{
			...
			if (editColors)
			{
				...
				{
					var bs = gameObject.GetComponentsInChildren<LayoutBorderDragger>();
					foreach (var b in bs) b.GetComponent<Image>().color = borderColor;
				}
				else
				{
