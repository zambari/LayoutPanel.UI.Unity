namespace zUI.LayoutPanelTools
{
	...
	public static class LayoutExt
	{
		...
		public static T[] GetComponentsInDirectChildren<T>(this MonoBehaviour mono, bool includeDisabled = true)
			where T : Component
		{
			return GetComponentsInDirectChildren<T>(mono.transform, includeDisabled);
		}

		/// <summary>
		/// Rearches one level of transform (doesn't go deep)
		/// </summary>
		public static T[] GetComponentsInDirectChildren<T>(this GameObject game, bool includeDisabled = true)
			where T : Component
		{
			return GetComponentsInDirectChildren<T>(game.transform, includeDisabled);
		}
		...
		/// </summary>
		/// 
		public static T[] GetComponentsInDirectChildren<T>(this Transform transform, bool includeDisabled = true)
			where T : Component
		{
		...
		public static LayoutElementState GetLayoutElementState(this LayoutElement layoutElement)
		{
			...
			if (layoutElement.flexibleHeight > 0 && layoutElement.flexibleHeight > 0)
				return LayoutElementState.layoutPresentStretch;

			if ((layoutElement.preferredHeight > 0 || layoutElement.minHeight > 0) &&
				(layoutElement.preferredWidth > 0 || layoutElement.minWidth > 0))
				return LayoutElementState.layoutNotFlexible;
		...
		public static void SetLayoutElementState(this LayoutElement layoutElement, LayoutElementState state)
		{
			...
			switch (state)
			{
				case LayoutElementState.noLayoutElement:
					...
					break;
				case LayoutElementState.layoutPresentIgnoring: layoutElement.ignoreLayout = true; break;
	...
	public static class LayoutExtensions
	{
		...
		*/
		...
		{
			RectTransform r = t.GetComponent<RectTransform>();
			if (r != null) return r.rect.height;
			else return -1;
		}
