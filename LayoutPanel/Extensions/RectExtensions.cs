namespace zUI.LayoutPanelTools
{
	...
	public static class RectExtensions
	{
		...
		public static void SetRelativeSizeX(this RectTransform rect, RectTransform parentRect, float v)
		{
			...
			if (v.CheckIfFloatNan()) rect.sizeDelta = new Vector2(sizeX * v, rect.sizeDelta.y);
		...
		public static void SetRelativeSizeY(this RectTransform rect, RectTransform parentRect, float v)
		{
			...
			if (v.CheckIfFloatNan()) rect.sizeDelta = new Vector2(rect.sizeDelta.x, sizeY * v);
			else Debug.Log("source is" + rect.name + " parent " + parentRect.name, rect.gameObject);
		}
		...
		{
			float sizeY = parentRect.rect.height;
			if (v.CheckIfFloatNan()) rect.localPosition = new Vector2(rect.localPosition.x, sizeY * v);
			else Debug.Log("source is" + rect.name + " parent " + parentRect.name, rect.gameObject);
		}
		...
		{
			float sizeX = parentRect.rect.width;
			if (v.CheckIfFloatNan()) rect.localPosition = new Vector2(sizeX * v, rect.localPosition.y);
			else Debug.Log("source is" + rect.name + " parent " + parentRect.name, rect.gameObject);
		}
		...
		{
			float sizeX = parentRect.rect.width;
			if (v.CheckIfFloatNan()) rect.offsetMin = new Vector2(sizeX * v, rect.offsetMin.y);
			else Debug.Log("source is" + rect.name + " parent " + parentRect.name, rect.gameObject);
		}
		...
		{
			float sizeX = parentRect.rect.width;
			if (v.CheckIfFloatNan()) rect.offsetMax = new Vector2(-sizeX * v, rect.offsetMax.y);
			else Debug.Log("source is" + rect.name + " parent " + parentRect.name, rect.gameObject);
		}
