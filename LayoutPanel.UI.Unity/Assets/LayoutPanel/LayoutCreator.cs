...
namespace zUI
{
    ...
    public class LayoutCreator : MonoBehaviour
    {
        ...
        {
            // var row = gameObject.GetComponent<LayoutColumn>();
            // if (row != null) DestroyImmediate(row);
            var vg = GetComponent<VerticalLayoutGroup>();
            if (vg != null) DestroyImmediate(vg);

            AddItemCreatorsToChildren(GetComponent<RectTransform>().SplitToLayout(true, splitCount));
            // gameObject.AddComponent<LayoutRow>();
        ...
        public void CreateVerticalLayout()
        {
            ...
            AddItemCreatorsToChildren(GetComponent<RectTransform>().SplitToLayout(false, splitCount));
        ...
        void AddItemCreatorsToChildren(List<GameObject> target)
        {
            for (int i = 0; i < target.Count; i++)
            {
                var thisChild = target[i];
                thisChild.AddOrGetComponent<LayoutItemCreator>();
        ...
        [ExposeMethodInEditor]
        public void CreateForAllUnparented()
        {
            ...
            foreach (var i in itemcr)
            {
                ...
                {
                    if (onlyCreateBorders) i.AddBordersOnly(); else i.ConvertToLayoutPanel();
                    processed++;
                }
                else
                    notprocssed++;
